...
The wrong path is reaching for a document because a document was mentioned, instead of naming the
quadrant first and letting the instrument follow from it — "name the unknown you are actually
holding, then let the instrument follow from the class. Getting the class wrong is how a project
ends up with an explainer nobody needed." A known known retires by prompt hygiene, not by paper:
"Say 'no document' when that is the answer... the one it is most tempted to skip." Classify before
...
interpolated or exported. Open all three entries in *What you read* below:
...
document  · <the chain document to write, "none — write the prompt instead", or "already built —
             direct document, no chain required">
```
...
6. **Say "no document" when that is the answer.** A known known is a prompt she failed to write, and
   ...
   **Say "already built — direct document, no chain required" when the subject exists, built and
   ...
   position entirely rather than routing to `write-chain-document`; it is not the same as "no
   document" — a document still gets written, just without licensing.
