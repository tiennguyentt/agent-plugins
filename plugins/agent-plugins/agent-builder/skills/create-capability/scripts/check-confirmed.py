...
`scripts/check.py --confirmed` in the tien-os workspace.
...
(`control-plane/GUARDRAILS.md` §9 rule 9: `Confirmed: <YYYY-MM-DD> — <signature>`, column 1 of
...
Licensed by
artifacts/workspace/pre-implementation/2026-07-31-spec-agent-builder-standalone.html §6, §11
step 4. If this file and `tien-os/scripts/check.py`'s `check_confirmed()` / `CONFIRMED_LINE_RE`
...
# Byte-identical to CONFIRMED_LINE_RE in tien-os/scripts/check.py. Column 1 of its own line —
# control-plane/GUARDRAILS.md §9 rule 9 fixes this as the one accepted format.
