...
// Upstream's `codex resume <threadId>` reattaches a long-lived Codex app-server thread.
// Our transport is `claude`, and the equivalent reattachment is `claude --resume <sessionId>`
// (verified in `claude --help`, 2026-08-10 — the flag exists, listed as
// "-r, --resume [value]  Resume a conversation by session ID").
function formatClaudeResumeCommand(job) {
  if (!job?.claudeSessionId) {
    return null;
  }
  return `claude --resume ${job.claudeSessionId}`;
}
...
function appendActiveJobsTable(lines, jobs) {
  ...
  for (const job of jobs) {
    ...
    lines.push(
      `| ${escapeMarkdownCell(job.id)} | ${escapeMarkdownCell(job.kindLabel)} | ${escapeMarkdownCell(job.status)} | ${escapeMarkdownCell(job.phase ?? "")} | ${escapeMarkdownCell(job.elapsed ?? "")} | ${escapeMarkdownCell(job.claudeSessionId ?? "")} | ${escapeMarkdownCell(job.summary ?? "")} | ${actions.map((action) => `\`${action}\``).join("<br>")} |`
    );
  }
}
...
export function renderTaskResult(parsedResult, meta) {
  ...
  const message = String(parsedResult?.failureMessage ?? "").trim() || "Claude did not return a final message.";
  return `${message}\n`;
}
...
export function renderStatusReport(report) {
  ...
  if (report.needsReview) {
    ...
    lines.push("Ending the session will trigger a fresh Claude adversarial review and block if it finds issues.");
  }
...
export function renderStoredJobResult(job, storedJob) {
  const claudeSessionId = storedJob?.claudeSessionId ?? job.claudeSessionId ?? null;
  const resumeCommand = claudeSessionId ? `claude --resume ${claudeSessionId}` : null;
  ...
  if (claudeSessionId) {
    lines.push(`Claude session ID: ${claudeSessionId}`);
    lines.push(`Resume in Claude: ${resumeCommand}`);
  }
