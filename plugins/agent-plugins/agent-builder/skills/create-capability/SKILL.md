...
description: >
  ...
  năng X", or "I need my OS to handle Y".
---
...
you produce specs, block files, rows in `control-plane/COMPONENT-CATALOG.md`, and eval skeletons. You never run what you build,
...
2. If no `.git` is found in that walk, or `control-plane/GUARDRAILS.md` does not exist at that
   `.git` root once found → **standalone mode.**
3. If `control-plane/GUARDRAILS.md` exists at that root → **workspace mode.**
...
`control-plane/GUARDRAILS.md`, `control-plane/DEFINITION-OF-DONE.md`, or
`control-plane/COMPONENT-CATALOG.md` outside this workspace to read — none of those travel with
the plugin, on purpose (`control-plane/GUARDRAILS.md` §0's precedence chain stays inside the
...
`control-plane/templates/`, kept honest by a drift check (`scripts/check.py` check 15) that runs
...
Every file this skill names must exist. Read these four in the target repo before doing anything
...
| `control-plane/GUARDRAILS.md`         | §3 hard bans, §9 naming — **every section is ratified as of 2026-07-26**, so there is no DRAFT tier left to discount |
| `control-plane/DEFINITION-OF-DONE.md`   | the checks a capability must pass to be done — 1-9 for a capability, 10-11 for shipping it                           |
| `control-plane/COMPONENT-CATALOG.md` | who already owns the responsibility being asked for                                                                  |

**If any of those four is missing, stop and say which one.** A dead path that looks fine is this
...
Six things, in this order, all as one proposal:
...
3. **The licensed block files** — this may be one agent plus multiple skill
   ...
   rubric
4. A **row in `control-plane/COMPONENT-CATALOG.md`**
5. An **evaluation skeleton** at `artifacts/capabilities/<name>/evaluation/`, with at least three refusal cases
6. A **report** — the decision, the files, the open questions, any conflict with the rules
...
```bash
python3 scripts/check.py --confirmed <name-or-spec-file>
```

Exit `0` and a `PASS` line mean a spec under `artifacts/capabilities/<name>/` (a
`<YYYY-MM-DD>-spec-<name>.*` file — date first, `control-plane/GUARDRAILS.md` §9 naming rule 1)
carries a valid `Confirmed:` line, the same acceptance `control-plane/DEFINITION-OF-DONE.md` check 2 runs against
...
`artifacts/workspace/pre-implementation/` holds the planning sequence for the tien-os workspace itself — the repo
...
`artifacts/capabilities/<name>/`, one folder per capability. A confirmed spec outranks this file, so use `artifacts/capabilities/`.
The alternative it rejected was flat `artifacts/workspace/pre-implementation/` told apart by filename; the reason given
...
Run the vendored checker instead of `python3 scripts/check.py --confirmed`:
...
`artifacts/capabilities/<name>/`, which is this workspace's own folder and does not exist outside
it.
...
- **Exit `1` →** refuse the build and scaffold instead of stopping bare. Write **zero files
  ...
  nothing resembling a `control-plane/COMPONENT-CATALOG.md` row, because there is no such file to
  edit outside this workspace. Run Phase 1's architecture check exactly as below to pick the
...
in `artifacts/workspace/pre-implementation/2026-07-23-explainer-evidence.html`. **"Tiên asked for an agent" is
...
- **Composed workflow** — keeping the steps fixed and the model's role small by composing the
  ...
  `control-plane/GUARDRAILS.md` §6's _"default to workflow, not agent"_ is about this sense.
...
(`control-plane/GUARDRAILS.md` §6) — work from the file, never from a remembered list. This instruction used to
...
Autonomy rules you may never override: everything ships **draft-only**; external comms in her name
are draft-only permanently; anything that spends, deletes, commits, or grants access is
approve-then-execute at minimum; the hard bans are not yours to waive.
...
your summary of either. `control-plane/GUARDRAILS.md` §6 and the mental model both forbid the doer grading its own
...
- **Skill** → `plugins/agent-plugins/<plugin-name>/skills/<name>/` — **a folder, up to three files**
- **Agent shell** → one logical role with a required packaged Claude adapter at
  `plugins/agent-plugins/<plugin-name>/agents/<name>.md`. A Codex project
...
- **Agent-plugin packaging** → one independent directory at
  `plugins/agent-plugins/<plugin-name>/`, exactly one logical agent, both host
...
```
plugins/agent-plugins/<plugin-name>/skills/<name>/
├── SKILL.md                  always
...
Claude plugin adapter stay under `plugins/agent-plugins/<plugin-name>/`.
...
- **Always write `tools:` on an agent.** Omitting it grants every tool the caller has — the
  ...
  `plugins/agent-plugins/agent-builder/agents/agent-builder.md` carries no
...
Add a row to `control-plane/COMPONENT-CATALOG.md`, filling its columns as they exist on disk right now. Sensitivity: plain
words; a tier letter only once `control-plane/GUARDRAILS.md` §4 is ratified. Status: `Proposed — pending
review`. **Never mark your own build Live** — the one time a builder-adjacent process did that in
this system, it violated the gate within the first hour and had to be publicly corrected.

### Phase 5 — Eval skeleton

Create `artifacts/capabilities/<name>/evaluation/case.yaml` with the case count and mix the confirmed spec's eval section states.
...
conflict with the rules you hit. Then assess the build against `control-plane/DEFINITION-OF-DONE.md`, check by check —
...
did not run: a verification that has not been run is a claim, not a check. Your build is a proposal. Tiên signs off before anything
goes live, and autonomy loosens one notch at a time afterward, with evidence.
...
- **Never mark a build Live, and never grant autonomy** beyond draft-only.
- **Never send, publish, post or commit.** You draft; Tiên sends.
- **Never waive a `control-plane/GUARDRAILS.md` §3 hard ban** — no financial transactions, no access-granting,
...
- **Never duplicate an existing owner.** Check `control-plane/COMPONENT-CATALOG.md` first and flag the overlap instead.
...
name the check you could not run. This is the same discipline Phase 6 applies to `control-plane/DEFINITION-OF-DONE.md`:
...
`CLAUDE.md` · `control-plane/GUARDRAILS.md` · `control-plane/DEFINITION-OF-DONE.md` · `control-plane/COMPONENT-CATALOG.md` · `control-plane/templates/agent-spec.md` ·
`control-plane/templates/skill-spec.md` · `control-plane/templates/workflow-spec.md` ·
`control-plane/templates/agent-plugin-spec.md` ·
`artifacts/workspace/pre-implementation/2026-07-23-explainer-evidence.html`
...
| File | Open it when |
|---|---|
| `plugins/agent-plugins/agent-builder/skills/evaluate-capability/rubric.md` | before any of the three stages — it is the grading standard, and it is what the separate verifier reads |
| `plugins/agent-plugins/agent-builder/skills/create-capability/create-capability.workflow.js` | when a stage returns something unexpected, or you need to know which tier it used |
...
Workflow({ scriptPath: "${CLAUDE_PLUGIN_ROOT}/skills/create-capability/create-capability.workflow.js",
           args: { stage: "done-checks", artifact: "<what was built>",
                   checks: "<abs path to control-plane/DEFINITION-OF-DONE.md>",
                   rubric: "${CLAUDE_PLUGIN_ROOT}/skills/evaluate-capability/rubric.md" } })
```
...
|---|---|---|---|
| **architecture** (Phase 1) | five mechanism checks plus one composition pass | `sonnet`, `effort: 'high'` on the measured Claude adapter | reasoning over `control-plane/GUARDRAILS.md` and the evidence explainer. A misread rule here mis-shapes everything downstream |
...
| **done-checks** (Phase 6) | one check from `control-plane/DEFINITION-OF-DONE.md` | **`sonnet`, `effort: low`** | same. Most will honestly return `NOT-RUN`, and that is the correct answer, not a gap |

**The audit stage reads the section list out of the template rather than carrying one.** This
script deliberately hardcodes no field list: `control-plane/GUARDRAILS.md` §6 says the templates own it, and the last
