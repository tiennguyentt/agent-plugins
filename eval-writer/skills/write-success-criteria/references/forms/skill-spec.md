...
**The description must name the sibling that owns the other side of every boundary it
touches — including siblings in other plugins.** A `Do NOT use…` clause that names only
skills inside the same plugin leaves every cross-plugin collision unrouted, and a collision
