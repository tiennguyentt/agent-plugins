...
**Open every file this skill names before writing anything.** Use the `Read` tool, not a shell — the
packaged agent grants `Read`, `Write`, `Glob` and `Grep` and no `Bash`, so an `ls` gate would be
unrunnable in the one context that matters most. Both entries in *What you read* below:
