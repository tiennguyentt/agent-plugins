...
Use these as starting templates for Claude Code `task` prompts or other Claude prompt
construction. Copy the smallest recipe that fits the task, then trim anything you do not need.
In `claude-rescue`, run diagnosis and fix-oriented recipes in write mode by default unless the
user explicitly asked for read-only behavior.
...
The task is not done at the first plausible fix.
"Done" means: the reported symptom is verified gone by re-running the check that caught it, not
just that the code compiles.
Do not stop after identifying the issue without applying and verifying the fix.
</completeness_contract>

<anti_shortcut_rules>
Fix the root cause in the source, not the check.
Do not special-case the failing test's exact input, hard-code its expected output, weaken an
assertion, or disable the test to make it pass.
</anti_shortcut_rules>
...
<candor_rules>
Report every finding, including low-confidence and low-severity ones.
Do not soften a finding to make the review read as more agreeable.
</candor_rules>
...
```xml
<task>
Diagnose why this existing prompt is underperforming and propose the smallest high-leverage
changes to improve it for Claude.
</task>
...
<verification_loop>
Before finalizing, make sure the revised prompt resolves the cited failure modes without adding
