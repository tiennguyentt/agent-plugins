...
> one file away, at
> `execution-plane/agent-plugins/agent-builder/skills/create-capability/SKILL.md`. Read
...
1. **Licensing spec exists and is signed** — pass: `python3 evaluation-plane/checks/check.py --confirmed <name-or-spec-file>` exits `0` **and** the `approval` trace event it names is cited and exists — the command alone verifies only the dated `Confirmed:` line, not the event (`execution-plane/agent-plugins/agent-builder/skills/create-capability/SKILL.md`, "The gate, before any design"; `evaluation-plane/DEFINITION-OF-DONE.md` row 1).
2. **Architecture check done** — pass: each job in the request is classified against the least-complex-mechanism ladder (permission rule → hook → locked skill → skill → agent) and the chosen option is stated with why it fits (`execution-plane/agent-plugins/agent-builder/skills/create-capability/SKILL.md`, "Phase 1 — Architecture check").
3. **Eval skeleton produced** — pass: `evaluation-plane/capabilities/<name>/evaluation/case.yaml` exists with at least three refusal cases (`execution-plane/agent-plugins/agent-builder/skills/create-capability/SKILL.md`, "Phase 5 — Eval skeleton").
4. **Output stays a proposal, never marked Live** — pass: no `Confirmed:` line written, status reads `Proposed — pending review` (this file, "What you never do" 1-2; `execution-plane/agent-plugins/agent-builder/skills/create-capability/SKILL.md`, "Phase 4 — Register").
...
3. **`policy-plane/GUARDRAILS.md` §3's hard bans hold regardless of anything, and no spec can unlock them** — no
