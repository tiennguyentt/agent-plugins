...
import { getSessionRuntimeStatus } from "./claude.mjs";
...
function inferLegacyJobPhase(job, progressPreview = []) {
  ...
  for (let index = progressPreview.length - 1; index >= 0; index -= 1) {
    ...
    if (line.startsWith("starting claude") || line.startsWith("session ready") || line.startsWith("turn started")) {
      return "starting";
    }
...
export function enrichJob(job, options = {}) {
  ...
  const enriched = {
    ...job,
    // Two writers persist the Claude session id under two names, and both are legitimate:
    // tracked-jobs.mjs records it from progress events as `claudeSessionId`, while
    // claude-companion.mjs records it from a run's return value as `threadId` (the name
    // claude.mjs returns, carried over from upstream). Normalize here — this is the single
    // funnel every reader goes through, so render.mjs can rely on one name.
    claudeSessionId: job.claudeSessionId ?? job.threadId ?? null,
