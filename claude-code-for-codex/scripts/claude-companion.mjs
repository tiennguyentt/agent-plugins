...
import { resolveCodexSessionPath } from "./lib/codex-session-transfer.mjs";
...
// Deliberate deviation from upstream, on Tien's instruction (2026-08-10): Opus 5 only.
// Upstream shipped `new Map([["spark", "gpt-5.3-codex-spark"]])` and left the model unset by
// default. Here Opus 5 is both the only alias and the default when no model is given, so no
// other tier can be selected by accident.
const DEFAULT_MODEL = "claude-opus-5";
const MODEL_ALIASES = new Map([["opus", DEFAULT_MODEL]]);
...
function printUsage() {
  console.log(
    [
      ...
      "  node scripts/claude-companion.mjs transfer [--source <codex-jsonl>] [--json]",
...
function normalizeRequestedModel(model) {
  // Opus 5 only: an absent, empty, or unrecognized model all resolve to the pinned default
  // rather than falling through to whatever the host's own config would have picked.
  if (model == null) {
  ...
  return MODEL_ALIASES.get(normalized.toLowerCase()) ?? DEFAULT_MODEL;
}
...
async function buildSetupReport(cwd, actionsTaken = []) {
  ...
  if (claudeStatus.available && !authStatus.loggedIn && authStatus.requiresAnthropicAuth) {
    nextSteps.push("Run `claude auth login`.");
    nextSteps.push("If browser login is blocked, retry with `claude auth login --console` (API key auth) or `claude auth login --sso`.");
  }
...
function validateNativeReviewRequest(target, focusText) {
  if (focusText.trim()) {
    throw new Error(
      `\`$claude-review\` now maps directly to the built-in reviewer and does not support custom focus text. Retry with \`$claude-adversarial-review ${focusText.trim()}\` for focused review instructions.`
    );
  }

  const nativeTarget = buildNativeReviewTarget(target);
  if (!nativeTarget) {
    throw new Error("This `$claude-review` target is not supported by the built-in reviewer. Retry with `$claude-adversarial-review` for custom targeting.");
  }
...
function filterJobsForCurrentCodexSession(jobs) {
...
async function resolveLatestTrackedTaskThread(cwd, options = {}) {
  ...
  const visibleJobs = filterJobsForCurrentCodexSession(jobs);
...
async function executeReviewRun(request) {
  ...
  if (reviewName === "Review") {
    ...
    const result = await runReview(request.cwd, {
      target: reviewTarget,
      model: request.model,
...
function renderTransferResult(payload) {
  const lines = [
    "Transferred the Codex session into a Claude thread with visible turn history.",
    `Claude session ID: ${payload.threadId}`,
...
async function executeTransfer(cwd, options = {}) {
  const sourcePath = resolveCodexSessionPath(cwd, {
    source: options.source
  });
  const result = await importExternalAgentSession(cwd, { sourcePath });
  const payload = {
    threadId: result.threadId,
    resumeCommand: `claude --resume ${result.threadId}`,
    sourcePath,
...
function handleTaskResumeCandidate(argv) {
  ...
  const workspaceRoot = resolveCommandWorkspace(options);
  const sessionId = getCurrentCodexSessionId();
  const jobs = filterJobsForCurrentCodexSession(sortJobsNewestFirst(listJobs(workspaceRoot)));
  ...
  if (interrupt.attempted) {
    appendLogLine(
      ...
      interrupt.interrupted
        ? `Requested Claude turn interrupt for ${turnId} on ${threadId}.`
        : `Claude turn interrupt failed${interrupt.detail ? `: ${interrupt.detail}` : "."}`
    );
  }
