...
**Open every file this skill names before answering.** Use `Read` and `Glob`, not a shell — the
packaged agent grants `Read`, `Write`, `Glob` and `Grep` and no `Bash`, so an `ls` gate would be
unrunnable in the one context that matters most. All six entries in *What you read* below.
