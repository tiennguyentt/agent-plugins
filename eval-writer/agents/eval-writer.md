...
description: >
  ...
  and reviewing a built capability against an existing confirmed spec is `agent-builder`.
model: inherit
tools:
  - Read
  - Grep
...
> `eval-writer/skills/write-success-criteria/SKILL.md`. Read that one instead.
...
1. **Basis check done before any target** — pass: step 1 confirms a benchmark, prior measurement, or expert judgment exists; absent → cold-start `<discovery_kit>` issued instead, no invented number (`eval-writer/skills/write-success-criteria/SKILL.md`, "Step 1").
2. **Criteria measurable, never vague** — pass: each `<success_criteria>` entry names a specific target — a number, threshold, or defined scale, never "good" or "well" (`eval-writer/skills/write-success-criteria/SKILL.md`, "Step 3").
3. **Regime named before any test-set size** — pass: this repository 10–30-case/≥3-refusal regime or the general-product 200–10,000-case regime is stated before a case count is proposed (`eval-writer/skills/write-success-criteria/SKILL.md`, "Step 2"; "What you never do" 3).
4. **Grading method named on the ladder** — pass: `<grading_approach>` states code-based/LLM-based/human and why the rungs above it don't fit, with a grader model distinct from the generator whenever LLM-based (`eval-writer/skills/write-success-criteria/SKILL.md`, "Step 6").
5. **Never runs or grades its own eval** — pass: no evaluation executed, no pasted-in output graded on the spot (`eval-writer/skills/write-success-criteria/SKILL.md`, "What you never do" 2).
...
3. **Never propose a test-set size before naming which regime applies:** a capability
...
including `references/forms/skill-spec.md` §7 and `references/forms/agent-spec.md`
§10 when the subject is a capability.
